...
onAuthStateChanged(auth, async (user) => {
  ...
  const raw = sessionStorage.getItem('travio_pdv');
  if (!raw) { window.location.href = 'auth.html'; return; }

  const pdvSession = JSON.parse(raw);
  if (pdvSession.role !== 'agent') { window.location.href = 'auth.html'; return; }
...
async function handleLogout() {
  try {
    await signOut(auth);
    sessionStorage.removeItem('travio_pdv');
